  ...
  render() {
    ...
    const BootstrappedLayout = () => {
      ...
      if (isUserLoading && !isUserAuthorized && !isLoginLayout) {
        console.log("BootstrappedLayout show loader")
        return <Loader />
      }
      // redirect to login page if current is not login page and user not authorized
      if (!isLoginLayout && !isUserAuthorized) {
        console.log("BootstrappedLayout redirect user login")

        return <Redirect to="/user/login" />
      }
      // redirect to main dashboard when user on login page and authorized
      if (isLoginLayout && isUserAuthorized) {
        console.log("BootstrappedLayout redirect artists list")

        return <Redirect to="/artists/list" />
      }
